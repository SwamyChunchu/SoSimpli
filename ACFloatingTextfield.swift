...
fileprivate extension ACFloatingTextfield {
    ...
    func addBottomLine(){
        ...
        bottomLineView = UIView(frame: CGRect(x:5, y:self.frame.height-1, width:self.frame.width-5, height:2))
    ...
    func resignPlaceholder() -> Void {
        ...
        let labelFrame = CGRect(x:4, y:2.5, width:self.frame.size.width-5, height:self.frame.size.height)

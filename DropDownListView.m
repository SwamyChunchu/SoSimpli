...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple
{
    ...
    if (aOptions.count < 4) {
        rect = CGRectMake(point.x, point.y+125, size.width, height);
    }
    if (self = [super initWithFrame:rect])
    {
        ...
        _kTableView = [[UITableView alloc] initWithFrame:CGRectMake(DROPDOWNVIEW_SCREENINSET,
             ...
             rect.size.height - 2 * DROPDOWNVIEW_SCREENINSET - DROPDOWNVIEW_HEADER_HEIGHT - RADIUS+5)];
        ...
        _kTableView.layer.shadowRadius = 2.0f;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect titleRect = CGRectMake(DROPDOWNVIEW_SCREENINSET + 10,
    DROPDOWNVIEW_SCREENINSET + 10 + 5,
    rect.size.width -  2 * (DROPDOWNVIEW_SCREENINSET + 10),30);
    
    CGRect separatorRect = CGRectMake(DROPDOWNVIEW_SCREENINSET,
    DROPDOWNVIEW_SCREENINSET + DROPDOWNVIEW_HEADER_HEIGHT - 2,

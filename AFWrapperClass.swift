...
// MARK:  Request POST URL With Urlsession(String Format):
    ...
    class func requestPostURLsessionWithArray(_ strURL : String, params : NSArray, success:@escaping (NSDictionary) -> Void, failure:@escaping (NSError) -> Void)
    {
                let urlwithPercentEscapes = strURL.addingPercentEncoding( withAllowedCharacters: CharacterSet.urlQueryAllowed)
    ...
    class func requestPostWithMultiFormData(_ strURL : String, params : [String : String]?, headers : [String : String]?, success:@escaping (NSDictionary) -> Void, failure:@escaping (NSError) -> Void){
        ...
        Alamofire.upload(multipartFormData: { (multipartFormData) in
           ...
           encodingCompletion: { encodingResult in
            ...
            case .failure(let encodingError):
                //print(encodingError)
                _ = encodingError.localizedDescription
...
//    {
//        let USerNameRegEx = "^(?=\\S{8})[a-zA-Z]\\w*(?:\\.\\w+)*(?:@\\w+\\.\\w{2,4})?$"
